...
/**
 * Notion 데이터베이스에서 견적서 목록 조회 (내부 함수)
 ...
 * @throws Error - 조회 실패 시
 */
async function fetchInvoicesFromNotion(
  pageSize: number = 10,
  startCursor?: string,
  ...
  try {
    ...
    // 정렬 속성 매핑 (띄어쓰기 없는 속성명 사용)
    const sortProperty = sortBy === 'issue_date' ? '발행일' : '총금액'
 ...
 * unstable_cache로 2분간 캐싱됩니다.
 */
const getCachedInvoicesFromNotion = createCachedInvoiceListFetcher(
  fetchInvoicesFromNotion
)

/**
 * 최적화된 견적서 목록 조회 (캐싱 적용)
 * 외부에서 사용하는 메인 함수
 *
 * @param pageSize - 페이지당 항목 수 (기본값: 10, 최대: 100)
 * @param startCursor - 페이지네이션 시작 커서
 ...
 * @example
 * ```typescript
 * const result = await getInvoicesFromNotion(10, undefined, 'issue_date')
 * ```
 */
export async function getInvoicesFromNotion(
  ...
  return getCachedInvoicesFromNotion(pageSize, startCursor, sortBy)
}
...
export async function searchInvoices(
  ...
  try {
    ...
    // 1. 클라이언트명 또는 견적서 번호 검색 (띄어쓰기 없는 속성명 사용)
    if (filters.query) {
      notionFilters.push({
        or: [
          {
