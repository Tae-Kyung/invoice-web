...
/**
 * 환경별 로그 레벨 필터링
 * 개발 환경에서는 모든 레벨, 프로덕션에서는 warn 이상만 출력
 */
function shouldLog(level: LogLevel): boolean {
  if (process.env.NODE_ENV === 'development') {
    return true // 개발 환경: 모든 로그 출력
  }

  // 프로덕션 환경: debug는 출력하지 않음
  return level !== 'debug'
}
...
export function log(
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    console[consoleMethod](`[${timestamp}] ${levelUpper}: ${message}`)
